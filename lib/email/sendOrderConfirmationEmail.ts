...
// // // lib/email/email.ts
// // import { Resend } from 'resend';
// // import { generateOrderConfirmationHtml, OrderConfirmationData } from './order-confirmation';

// // const resend = new Resend(process.env.RESEND_API_KEY!);

// // export async function sendOrderConfirmationEmail(data: OrderConfirmationData & { to: string }) {
// //   const { to, ...orderData } = data;
// //   const recipient = process.env.TEST_EMAIL || to;
...
export async function sendOrderConfirmationEmail(data: OrderConfirmationData & { to: string }) {
  ...
  // 1. Generate the HTML as a string (No React components used here)
  const htmlContent = generateOrderConfirmationHtml(orderData);

  try {
    const { data: resendData, error } = await resend.emails.send({
      from: 'ShopNow <onboarding@resend.dev>',
      to: [recipient],
      subject: `Order Confirmation #${orderData.orderNumber}`,
      html: htmlContent, // Sending as RAW HTML string
    });
...
export async function sendResetPasswordEmail(email: string, token: string) {
  ...
  const html = `
    <div style="font-family: sans-serif; padding: 20px;">
      ...
      <a href="${resetUrl}" style="background: #2563eb; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block;">
        Reset Password
      </a>
    </div>
  `;
